...
export const instance = axios.create({
    baseURL:
        process.env.NODE_ENV === 'production' ? 'https://api.likeyun.net' : '',
...
const volume = {
    ...
    removeVolumeProblem: (data: any) =>
        request.del(`/volumes/problem-delete/${data.volumeId}/${data.id}`),
    updateVolumeProblem: (data: any) =>
        request.put('/volumes/problem/update', data),
    updateVolumeName: (data: any) => request.put('/volumes/update-name', data),
    getVolumeProblemAll: (id: number) =>
    ...
    updateVolumeOutline: (data: any) =>
        request.put('/volumes/outline/update', data),
...
const course = {
    ...
    getPreviewFinished: (id: string) =>
        request.get(`/course/preview/finished/${id}`),
    ...
    getExaminationFinished: (id: string) =>
        request.get(`/course/examination/finished/${id}`),
    ...
    getStudentTestProblem: (data: any) =>
        request.get(`/course/student-test/${data.testId}/${data.id}`),
...
const lore = {
    getProblemLoreLlist: (problemId: number) =>
        request.get(`/problem-lore/list/${problemId}`),
    postProblemLore: (data: any) =>
        request.post(
            `/problem/${data.problemId}/lores/${data.loreId}/${data.problemType}`
        ),
