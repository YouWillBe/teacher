...
const OptionA: FC = props => {
    ...
    const handleClickAnswerNumber = (text: string, index: number) => {
        if (text === '加') {
            ...
            volumeStore.currentAnswer.push({
                statu: false,
                name: answerOption[index],
            })
            volumeStore.volumeProblem.option = [
                ...volumeStore.volumeProblem.option,
                {
                    id: index + 1,
            ...
            volumeStore.volumeProblem.option.splice(index - 1, 1)
            volumeStore.currentAnswer.splice(index - 1, 1)
        }
    }

    //选择答案
    const handleClickAnswer = (index: number) => {
        volumeStore.volumeProblem.option.map((x: ICurrentAnswer, y: number) => {
            if (volumeStore.volumeProblem.type === 1) {
                if (y === index) {
                    x.statu = !x.statu
                } else {
                    x.statu = false
